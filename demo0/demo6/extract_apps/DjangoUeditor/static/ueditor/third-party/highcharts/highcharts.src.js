...
function stableSort(arr, sortFunction) {
	...
	// Add indexes to each item
	for (i = 0; i < length; i++) {
		arr[i].ss_i = i; // stable sort indexes
	}
	...
	// Remove indexes from items
	for (i = 0; i < length; i++) {
		delete arr[i].ss_i; // stable sort indexes
	}
}
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		/**
		 * Return the indexes of an item in an array, or -1 if not found
		 */
		inArray: $.inArray,
...
SVGElement.prototype = {
	...
	*/
	...
	add: function (parent) {
		...
		if (parentWrapper.handleZ) { // this element or any of its siblings has a z indexes
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// This will keep each points' extremes stored by series.indexes
	this.points = {};
...
Axis.prototype = {
	...
	remove: function (redraw) {
		...
		each(chart[key], function (axis, i) { // Re-indexes, #1706
...
Pointer.prototype = {
	...
	/**
	 * Return the indexes in the tooltipPoints array, corresponding to pixel position in
	 * the plot area.
	 */
	getIndex: function (e) {
		var chart = this.chart;
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		// Sort series according to indexes option (#248, #1123)
		...
		while (i--) {
			...
			if (distanceOption > 0) {
				...
				for (j = 0; j < length; j++) {
					...
					// if that slot indexes is closer to the edges of the slots, move it
